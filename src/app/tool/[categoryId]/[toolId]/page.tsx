  ...
  "word-counter": {
    ...
    settings: [
      ...
      },
      {
        type: "switch",
        id: "countPunctuation",
        label: "Count punctuation separately",
        default: false,
      },
    ],
  },
    ...
    settings: [
      {
        type: "switch",
        id: "addBookmarks",
        label: "Add bookmarks for each file",
        default: true,
      },
      {
        type: "switch",
 ...
 *
 * Dynamically renders the interface for various tools based on the category and tool IDs.
 * Handles file uploads, processing simulation, and settings customization.
 */
export default function ToolPage() {
  const params = useParams();
  const { user, checkUsage, incrementUsage } = useAuth();
  ...
  const handleProcess = () => {
    if (!user) {
      toast.error("Please login to process files");
    ...
    const interval = setInterval(() => {
      setProgress((prev) => {
        if (prev >= 100) {
          ...
          setProcessing(false);
          setCompleted(true);
          incrementUsage(); // Increment usage on completion
      ...
      <div className="max-w-7xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <Link
            ...
            className="hover:text-foreground transition-smooth capitalize"
          >
            {categoryId} Tools
          </Link>
          <ArrowRight className="h-4 w-4" />
